...
1. **Prepare the Release**
   ...
   - Re-pin the cross-reference URLs in `spec/openapi.yaml` to the tag being
     released, so a released specification links to its own prose rather than
     to whatever `main` says later:

     ```bash
     git grep -l 'blob/v[0-9]*\.[0-9]*\.[0-9]*/' -- spec \
