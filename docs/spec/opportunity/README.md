...
- `limit` is supplied by the client on the initial request. It is an integer no
  smaller than 1, and as everywhere else in STAPI it is a maximum rather than
  an exact count. This specification sets no upper bound and no default page
  size; both are implementation concerns.
